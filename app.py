...
**Contact Us:** [✉️ aramide.adebesin@gmail.com](mailto:adebesinaramide@gmail.com)
""")
    ...
    # Medication & Time with None option
    ...
    # Meals selection
    meals = st.multiselect("Meals eaten today:", [
        "Eba","Amala","Semovita","Fufu","Pounded Yam","Tuwo","Wheat",
        "White Rice","Jollof Rice","Fried Rice","Ofada Rice","Coconut Rice",
        "Oats","Pap","Custard","Spaghetti","Noodles","Corn Meal","Beans",
        "Moi Moi","Akara","Boiled Egg","Fried Egg","Boiled Chicken","Fried Chicken",
        "Grilled Fish","Boiled Fish","Goat Meat","Beef","Snail","Liver","Cow Skin (Ponmo)",
        "Boiled Yam","Yam Porridge","Fried Yam","Boiled Potato","Sweet Potato","Fried Plantain",
        "Boiled Plantain","Roasted Plantain (Boli)","Boiled Corn","Egusi Soup","Okro Soup",
        "Ogbono Soup","Efo Riro","Afang Soup","Edikang Ikong","Vegetable Soup",
        "Bitterleaf Soup","Gbegiri","Ewedu","Banga Soup","Groundnut Soup","Pepper Soup",
        "Garden Egg Sauce","Tomato Stew","Palm Oil Sauce","Fish Sauce","Egg Sauce","Zobo",
        "Smoothies","Juice","Soft Drinks","Water","Energy Drinks","Milk"
    ])

    # Trigger causes
    trigger_causes = st.multiselect("Possible triggers today:", [
        "Spicy foods","Tomatoes","Citrus fruits","Fried foods","Fatty foods",
        "Dairy","Chocolate","Caffeinated drinks","Carbonated drinks","Alcohol",
        "Fermented foods","Raw onions/garlic","Skipped a meal","Ate late",
        "Overate","Ate under stress","Took NSAIDs","Stress"
    ])

    # Flags
    ate_triggers = st.radio("Ate known trigger foods?", ["Yes","No"])
    skipped_meal = st.radio("Skipped meals today?", ["Yes","No"])
    ...
    # Cancer & H. pylori
    cancer_diag = st.radio("Diagnosed with gastric cancer?", ["Yes","No"])
    family_history = st.radio("Family history of gastric cancer?", ["Yes","No","Not sure"])
    h_pylori_ulcer = st.radio("Ulcer caused by H. pylori?", ["Yes","No"])
